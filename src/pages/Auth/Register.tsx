                    ...
                    {step === 1 && (
                        ...
                        >
                            ...
                            <form onSubmit={handleEmailPassSignup} className="space-y-4">
                                ...
                                <button type="submit" disabled={!captchaToken || isSubmitting} className="w-full bg-primary hover:bg-primary-hover text-white font-bold py-3 rounded-xl shadow-lg shadow-primary/30 transition-all mt-2 disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center gap-2">
                            ...
                            <div className="mt-6">
                                ...
                                <button onClick={handleGoogleSignup} disabled={!captchaToken || isSubmitting} className="w-full bg-white/10 hover:bg-white/20 border border-white/20 text-white font-bold py-3 rounded-xl transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed shadow-[0_4px_15px_rgba(255,255,255,0.05)]">
                            ...
                            <form onSubmit={finishRegistration}>
                                <div className="space-y-4 mb-8">
                                    <div className="relative">
                                        ...
                                        <input type="text" value={visualNick} onChange={e => setVisualNick(e.target.value)} placeholder="Visual Nickname (e.g. ProServerOwner)" required className="w-full bg-surface border border-white/10 rounded-xl py-3 pl-11 pr-4 outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all text-white disabled:opacity-50" disabled={isSubmitting} />
                                    </div>
                                    <p className="text-xs text-textMuted text-center">This is how you will appear to others on the platform.</p>
                                </div>

                                <button type="submit" disabled={isSubmitting} className="w-full py-3 rounded-xl bg-primary hover:bg-primary-hover text-white font-semibold shadow-lg shadow-primary/25 transition-all text-lg mb-4 disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center gap-2">
                                    {isSubmitting ? <Loader2 className="animate-spin" size={20} /> : 'Finish & Go to Dashboard'}
                                </button>
                            </form>
                        </motion.div>
                    )}
