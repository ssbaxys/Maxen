...
import { Mail, Lock, Phone as PhoneIcon } from 'lucide-react';
            ...
            <div className="glass-panel w-full max-w-md p-8 rounded-3xl relative z-10 animate-slide-up">
                ...
                {method === 'email' && (
                    <form onSubmit={handleEmailLogin} className="space-y-4">
                        <div className="relative">
                            ...
                            <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="Email / ID" required className="w-full bg-surface border border-white/10 rounded-xl py-3 pl-11 pr-4 outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all text-white" />
                        </div>
                        <div className="relative">
                            <Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-textMuted w-5 h-5" />
                            <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Password" required className="w-full bg-surface border border-white/10 rounded-xl py-3 pl-11 pr-4 outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all text-white" />
                        </div>
                        <button type="submit" className="w-full bg-primary hover:bg-primaryHover text-white font-semibold py-3 rounded-xl shadow-lg shadow-primary/25 transition-all">Sign In</button>
                    </form>
                )}
                ...
                {method === 'passwordless' && (
                    <form onSubmit={handlePasswordless} className="space-y-4">
                        ...
                        <button type="submit" className="w-full bg-accent hover:bg-yellow-500 text-black font-semibold py-3 rounded-xl shadow-[0_0_15px_rgba(245,158,11,0.3)] transition-all">Send Magic Link</button>
                    </form>
                )}
                        ...
                        {!showOtp ? (
                            <>
                                <div className="relative">
                                    ...
                                    <input type="tel" value={phone} onChange={e => setPhone(e.target.value)} placeholder="+1234567890" required className="w-full bg-surface border border-white/10 rounded-xl py-3 pl-11 pr-4 outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all text-white" />
                                </div>
                                <div id="recaptcha-container"></div>
                                <button type="submit" className="w-full bg-secondary hover:bg-emerald-400 text-white font-semibold py-3 rounded-xl shadow-lg shadow-secondary/25 transition-all">Send OTP</button>
                            </>
                        ) : (
                            <>
                                <input type="text" value={otp} onChange={e => setOtp(e.target.value)} placeholder="Enter 6-digit OTP" required className="w-full bg-surface border border-white/10 rounded-xl py-3 px-4 text-center tracking-widest outline-none focus:border-secondary transition-all text-white text-xl" />
                                <button type="submit" className="w-full bg-secondary hover:bg-emerald-400 text-white font-semibold py-3 rounded-xl shadow-lg shadow-secondary/25 transition-all">Verify OTP</button>
                            </>
                        )}
                    </form>
                )}
                ...
                <div className="mt-6">
                    ...
                    <button onClick={handleGoogleLogin} className="w-full bg-white/5 hover:bg-white/10 border border-white/10 text-white font-medium py-3 rounded-xl transition-all flex items-center justify-center gap-2">
